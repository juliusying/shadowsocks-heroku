...
(function() {
  var Encryptor, KEY, LOCAL_ADDRESS, METHOD, PORT, REMOTE_PORT, SCHEME, SERVER, WebSocket, config, configContent, configFromArgs, fs, getServer, https, inetNtoa, k, net, options, parseArgs, path, prepareServer, ref, s, server, timeout, url, v, HttpsProxyAgent, 
   proxy, WSOPTS, agent;
  ...
  proxyopts.secureEndpoint = true;
  
  var pfxpath = config.pfx_path;
  var pfxpass = config.pfx_pass;
  ...
  if(proxy !== "null" && proxy !== ""){
    WSOPTS.agent = new HttpsProxyAgent(proxyopts);
  }
    ...
    connection.on("data", function(data) {
      ...
      if (stage === 1) {
        try {
          ...
          ws = new WebSocket(aServer, WSOPTS);
          console.log("trying to open " + aServer);
          ws.on("open", function() {
